...
active_scene = None
scene_start_time = 0
preview_mode = False # If True, editor changes update lights immediately
...
def hardware_loop():
    ...
    # Mode A: Playing a Scene
    if active_scene:
        elapsed_mins = (time.time() - scene_start_time) / 60.0
        duration = active_scene.get('duration_mins', 60)
        
        # Stop if done
        if elapsed_mins > duration:
            elapsed_mins = duration
    ...
    # Mode B: Live Preview (Manual Control)
    elif preview_mode:
        # Just use the currently selected point in the editor as the "Live" value
        # This allows you to drag the slider and see the light change
        points_int = editing_scene['intensity_points']
        points_col = editing_scene['color_points']
        
        # Use the active point index or just the first one if unsure
        # For simplicity, we calculate based on the time slider position in the editor if possible
        # But simpler: just take the value of the active point being edited
        if active_point_idx < len(points_int) and active_point_idx < len(points_col):
             # This is a bit tricky since indices might not match.
             # Let's just use the current UI slider values if they exist
             pass # Logic handled inside on_param_change for responsiveness
...
def voice_listener_thread():
    ...
    print("[Voice] Listening for Scene Names, 'Lumos', 'Nox'")
        ...
        try:
            with mic as source:
                ...
                if "lumos" in command:
                    ...
                    global active_scene, preview_mode
                    active_scene = None
                    preview_mode = False
                    continue
                ...
                # 2. Scene Activation (Name Matching)
                # Check custom scenes first, then presets
                all_scenes = custom_scenes + presets
                found = False
                for p in all_scenes:
                    # Check if the scene name appears in the spoken sentence
                    if p['name'].lower() in command:
                        activate_scene(p)
                        found = True
                        break
                
                if not found and "activate" in command:
                    ui.notify("Scene not found")
...
def activate_scene(scene_data):
    global active_scene, scene_start_time, preview_mode
    active_scene = copy.deepcopy(scene_data)
    scene_start_time = time.time()
    preview_mode = False # Disable manual control when playing a scene
    ui.notify(f"Playing Scene: {active_scene['name']}")
...
def on_param_change(param, value):
    ...
    # Live Preview Logic
    if preview_mode:
        send_live_preview_update()

def send_live_preview_update():
    # Only useful if we are dragging a point and want to see the color NOW
    # We grab the current value of the active point and send it
    try:
        # Get Current Intensity (from whatever point is active or just the first)
        # To be robust, let's just calc based on the graph state? 
        # Actually, simpler: Use the values from the slider directly
        hue = 0
        intensity = 100
        
        # This is a simplification for manual control:
        # If editing Color graph -> Send that color at 100% intensity
        # If editing Intensity graph -> Send White at that intensity
        
        if active_graph_key == 'color':
            p_col = editing_scene['color_points'][active_point_idx]
            hue = p_col[1]
            intensity = 100 # Assume bright when picking color
        else:
            p_int = editing_scene['intensity_points'][active_point_idx]
            intensity = p_int[1]
            hue = 0 # White
...
def toggle_preview():
    global preview_mode, active_scene
    preview_mode = not preview_mode
    if preview_mode:
        active_scene = None # Stop playing scene
        ui.notify("Preview Mode: ON (Move sliders to change light)")
    else:
        ui.notify("Preview Mode: OFF")
        ...
        with ui.tab_panel(tab_edit):
            with ui.card().classes('w-full p-4 bg-white mb-4'):
                with ui.row().classes('w-full items-center gap-4'):
                    ...
                    ui.button('LIVE PREVIEW', on_click=toggle_preview).props('toggle flat dense').classes('text-xs')
                    ui.button('TEST SCENE', icon='play_arrow', on_click=lambda: activate_scene(editing_scene)).props('unelevated color=black')
