...
default_state = {
    ...
    'intensity_points': [[0, 0], [540, 0]], 'color_points': [[0, 0], [540, 0]],
...
def init_serial():
    ...
    for port in ports:
        try:
            ...
            pass
    print("[NEOPIXEL] WARNING: Arduino not found. Running in UI-only mode.")
    return None
...
def preview_current_point_to_led():
    ...
    # Need valid intensity & color curves
...
def load_scenes_from_file():
    ...
    if os.path.exists(SAVE_FILE):
        try:
            ...
            # MIGRATE: ensure every scene has all keys from default_state
            migrated = []
            for s in loaded:
                # start from default, then override with saved values
...
def on_panel_change(source, value):
    ...
    # Update LEDs for slider/color changes
    preview_current_point_to_led()
    ...
    with ui.column().classes('w-full max-w-3xl mx-auto'):
        ...
        with ui.row().classes('w-full gap-8 items-start'):
            with ui.column().classes('flex-grow'):
                ...
                ui_refs['slider_time'] = ui.slider(min=0, max=540, step=1,
                                                   on_change=lambda e: on_panel_change('time', e.value)).props('label-always color=black')
            with ui.column().classes('flex-grow'):
                ui.label('VALUE SETTING').classes('text-[10px] font-bold text-gray-400')
                ui_refs['slider_val'] = ui.slider(min=0, max=100, step=1,
                                                  on_change=lambda e: on_panel_change('value', e.value)).props('label-always color=blue')
                with ui.element('div').classes('hidden w-full') as container:
                    ...
                    ui_refs['color_picker_element'] = ui.element(
                        ...
                        'update:model-value', lambda e: on_panel_change('color', e.args)
                    )
