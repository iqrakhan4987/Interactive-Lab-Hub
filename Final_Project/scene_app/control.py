...
from hardware import send_color  # <-- handles Arduino + NeoPixel
...
default_state = {
    ...
    'end_time': '07:00',
    'duration_mins': 540,
    'intensity_points': [[0, 0], [540, 0]],
    'color_points': [[0, 0], [540, 0]],
    'volume_points': [[0, 0], [540, 0]],
    'sound_track': 'Silence',
    'loop_audio': True,
}
...
ui_refs = {
    ...
    'slider_time': None,
    'slider_val': None,
    'color_picker_container': None,
    'color_picker_element': None,
    'point_label': None,
    'tabs': None,
    'tab_studio': None,
    'tab_my_scenes': None,
    'my_scenes_container': None,
    'track_select': None,
    'upload_dialog': None,
...
def interpolate_points(points, t):
    """Linear interpolation of a time->value series."""
    if not points:
        return 0
...
def preview_current_point_to_led():
    """Use current scene + selected point to update the NeoPixel ring."""
    global active_graph_key, active_point_idx

    if not editing_scene.get('intensity_points') or not editing_scene.get('color_points'):
        return

    # Get current time from active graph's active point
    points = editing_scene.get(f'{active_graph_key}_points', [])
    if not points or active_point_idx >= len(points):
        return

    t = float(points[active_point_idx][0])
    duration = max(1, editing_scene.get('duration_mins', 1))
    t = max(0, min(t, duration))

    # Interpolate intensity (0–100) and hue (0–360)
    intensity = interpolate_points(editing_scene['intensity_points'], t)
    hue = interpolate_points(editing_scene['color_points'], t)
...
def load_scenes_from_file():
    """Load scenes and migrate old versions to the latest schema."""
    ...
    if os.path.exists(SAVE_FILE):
        try:
            ...
            migrated = []
            for s in loaded:
                new_s = copy.deepcopy(default_state)
                new_s.update(s)  # old scenes might not have all keys
                migrated.append(new_s)

            custom_scenes = migrated
        except Exception as e:
            print(f"[SCENES] Error loading scenes: {e}")
            custom_scenes = []
    else:
        custom_scenes = []
...
def save_scenes_to_file():
    with open(SAVE_FILE, 'w') as f:
        json.dump(custom_scenes, f, indent=4)
...
def update_context_panel():
    ...
    try:
        visual_order = sorted_indices.index(active_point_idx) + 1
    except ValueError:
        visual_order = active_point_idx + 1
    ...
    if active_point_idx < len(points):
        ...
        if active_graph_key == 'color':
            if ui_refs['color_picker_container']:
                ui_refs['color_picker_container'].classes(remove='hidden')
            if ui_refs['slider_val']:
                ui_refs['slider_val'].classes(add='hidden')
            hex_col = hue_to_hex(point[1])
            if ui_refs['color_picker_element']:
                ui_refs['color_picker_element'].props(f'model-value="{hex_col}"')
        else:
            if ui_refs['color_picker_container']:
                ui_refs['color_picker_container'].classes(add='hidden')
            if ui_refs['slider_val']:
                ui_refs['slider_val'].classes(remove='hidden')
                ui_refs['slider_val'].value = point[1]

    # Update LEDs when the context panel changes selection
    preview_current_point_to_led()
...
def close_context_panel():
    if ui_refs['footer']:
        ui_refs['footer'].classes(remove='translate-y-0', add='translate-y-full')


def show_context_panel():
    if ui_refs['footer']:
        ui_refs['footer'].classes(remove='translate-y-full', add='translate-y-0')


def reset_to_default():
    editing_scene.update(copy.deepcopy(default_state))
    editing_scene['id'] = None
    if ui_refs['scene_name_input']:
        ui_refs['scene_name_input'].value = 'New Scene'
    calculate_duration()
    ui.notify("Started New Scene")
    # Turn LEDs off for a new blank scene
    send_color(0, 0, 0)
...
def load_preset(preset):
    ...
    if 'intensity_points' in preset:
        editing_scene['intensity_points'] = [p[:] for p in preset['intensity_points']]
    if 'color_points' in preset:
        editing_scene['color_points'] = [p[:] for p in preset['color_points']]
    if ui_refs['scene_name_input']:
        ui_refs['scene_name_input'].value = editing_scene['name']
...
def handle_upload(e: events.UploadEventArguments):
    ...
    if ui_refs['upload_dialog']:
        ui_refs['upload_dialog'].close()
    ...
    if ui_refs['my_scenes_container']:
        ...
        with ui_refs['my_scenes_container']:
            if not custom_scenes:
                ui.label("No saved scenes yet.").classes(
                    'text-gray-400 italic w-full text-center mt-8'
                )
            for s in custom_scenes:
                with ui.card().classes(
                    'w-full p-4 border border-gray-200 shadow-sm flex-row justify-between items-center'
                ):
                    with ui.column().classes('gap-0'):
                        ui.label(s['name']).classes('font-bold text-lg')
                        start = s.get('start_time', default_state['start_time'])
                        end = s.get('end_time', default_state['end_time'])
                        ui.label(f"{start} - {end}").classes('text-xs text-gray-500')
                    with ui.row().classes('gap-2'):
                        ui.button(
                            icon='edit',
                            on_click=lambda _, x=s: load_scene_into_editor(x),
                        ).props('flat round dense')
                        ui.button(
                            icon='delete',
                            on_click=lambda _, x=s: delete_custom_scene(x['id']),
...
with ui.column().classes('w-full max-w-3xl mx-auto p-4 md:p-8 gap-6 pb-64'):
    with ui.row().classes('w-full justify-between items-center'):
        ui.label('SCENE STUDIO').classes(
            'text-2xl font-light tracking-widest text-gray-800'
        )
    ...
    with ui.tab_panels(tabs, value=tab_library).classes(
        ...
        with ui.tab_panel(tab_library):
            with ui.grid(columns=3).classes('w-full gap-4'):
                for p in presets:
                    ...
                    ).style(f'border-color: {color}').on(
                        'click', lambda _, x=p: load_preset(x)
                    ):
        ...
        with ui.tab_panel(ui_refs['tab_studio']):
            with ui.card().classes(
                'w-full p-6 rounded-b-xl rounded-tr-xl bg-white shadow-sm mb-6'
            ):
                with ui.row().classes(
                    'w-full justify-between items-center mb-4'
                ):
                    ui.label('SCENE SETTINGS').classes(
                        'text-xs font-bold text-gray-400'
                    )
                    ui.button(
                        'New Scene', icon='delete_sweep', on_click=reset_to_default
                    ).props('flat dense size=sm color=red')
                with ui.row().classes('w-full items-end gap-6'):
                    ui_refs['scene_name_input'] = ui.input('Name').bind_value(
                        editing_scene, 'name'
                    ).classes('text-lg font-bold flex-grow')
                    ui.input('Start').bind_value(
                        editing_scene, 'start_time'
                    ).props('type=time').on(
                        'update:model-value', calculate_duration
                    )
                    ui.label('→').classes('pb-4 text-gray-400')
                    ui.input('End').bind_value(
                        editing_scene, 'end_time'
                    ).props('type=time').on(
                        'update:model-value', calculate_duration
                    )
                    with ui.column().classes(
                        'items-center bg-gray-100 px-4 py-2 rounded-lg'
                    ):
            ...
            ui.label('CLICK A GRAPH TO EDIT').classes(
                'text-xs font-bold text-gray-400 tracking-widest'
            )
            active_graphs.append(
                DraggableGraph("VISUAL INTENSITY", "intensity", "orange")
            )
            active_graphs.append(
                DraggableGraph("COLOR (HUE)", "color", "purple")
            )
            active_graphs.append(
                DraggableGraph("AMPLITUDE", "volume", "blue")
            )

            with ui.card().classes(
                'w-full p-4 bg-gray-900 text-white shadow-sm mt-6'
            ):
                with ui.row().classes(
                    'w-full justify-between items-center'
                ):
                    with ui.column().classes('flex-grow pr-4'):
                        ui.label('AUDIO TRACK').classes(
                            'text-[10px] font-bold text-gray-500'
                        )
                        ui_refs['track_select'] = ui.select(
                            track_options, value='Morning Birds'
                        ).bind_value(
                            editing_scene, 'sound_track'
                        ).props(
                    ...
                    ui.button(
                        'Import',
                        icon='upload',
                        on_click=lambda: ui_refs['upload_dialog'].open(),
                    ).props('outline dense color=white size=sm')
                    ui.switch('Loop', value=True).props('color=green')

            ui.button('SAVE SCENE', on_click=save_scene).classes(
                'w-full mt-4 py-3'
            ).props('unelevated color=black')
...
with ui.dialog() as upload_dialog, ui.card():
    ...
    ).props('accept=".mp3, .wav"').classes('max-w-full')
    ui.button('Close', on_click=upload_dialog.close).props('flat')

with ui.footer().classes(
    'bg-white border-t border-gray-200 p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] transition-transform duration-300 translate-y-full'
) as footer:
    ...
    with ui.column().classes('w-full max-w-3xl mx-auto'):
        with ui.row().classes(
            ...
            ui_refs['point_label'] = ui.label('EDITING POINT').classes(
                'text-xs font-bold tracking-widest text-gray-400'
            )
            ui.button(
                'Done', icon='check', on_click=close_context_panel
            ).props('flat dense size=sm color=green')
        with ui.row().classes('w-full gap-8 items-start'):
            with ui.column().classes('flex-grow'):
                ui.label('TIME (MINUTES)').classes(
                    'text-[10px] font-bold text-gray-400'
                )
                ui_refs['slider_time'] = ui.slider(
                    min=0,
                    max=540,
                    step=1,
                    on_change=lambda e: on_panel_change('time', e.value),
                ).props('label-always color=black')
            with ui.column().classes('flex-grow'):
                ui.label('VALUE SETTING').classes(
                    'text-[10px] font-bold text-gray-400'
                )
                ui_refs['slider_val'] = ui.slider(
                    min=0,
                    max=100,
                    step=1,
                    on_change=lambda e: on_panel_change('value', e.value),
                ...
                with ui.element('div').classes('hidden w-full') as container:
                    ...
                    ui_refs['color_picker_element'] = ui.element(
                        'q-color'
                    ).props(
                        'no-header no-footer default-view=spectrum format-model=hex class="shadow-0 w-full"'
                    ).on(
                        'update:model-value',
                        lambda e: on_panel_change('color', e.args),
                    )
...
# If 8080 is busy you can change the port here (e.g., port=8081)
